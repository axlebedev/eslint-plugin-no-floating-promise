    ...
    create(context) {
        ...
        return {
            ...
            "CallExpression"(node) {
                ...
                const callee = node.parent.expression?.callee ?? node.callee;
                ...
                if (callee.type === "Identifier") {
                  const scope = sourceCode.getScope?.(node) ?? context.getScope()
                    const variable = findInScope(scope, callee.name);
                    if (variable === null || typeof variable === "undefined") {
                        return;
                    }

                    if (variable.async || variable.init?.async || getTypeAnnotation(variable) === "Promise") {
                        context.report({
                            fix(fixer) {
                                return fixer.replaceText(node, `await ${sourceCode.getText(node)}`);
                            },
                            loc: node.loc,
                            messageId: "foundFloating",
                            node
